...
function draw() {
  ...
  // Move particles and attract them to the mouse position
...
class Particle {
  static RAD = 4;
  static BOUNCE = -1;
  static SPEED_MAX = 2.4; // Increased maximum speed for faster movement
  static DIST_MAX = 50;
  ...
  attract(mx, my) {
    ...
    distance = constrain(distance, 5, 25); // Prevent too strong forces
    force.normalize();
    let strength = map(distance, 0, 25, 10, 0); // Increased attraction strength
    force.mult(strength);
