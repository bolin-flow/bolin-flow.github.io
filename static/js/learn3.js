...
function initParticles() {
  particles = [];
  let numParticles = map(windowWidth * windowHeight, 0, 1920 * 1080, 50, 200); // Adjust number of particles based on window size
...
function drawTriangles() {
  noStroke();
  fill(myColor.R, myColor.G, myColor.B, 8); // Adjust transparency
  stroke(max(myColor.R - 15, 0), max(myColor.G - 15, 0), max(myColor.B - 15, 0), 6); // Adjust transparency
...
class Particle {
  ...
  static DIST_MAX = 60; // Adjusted distance max
